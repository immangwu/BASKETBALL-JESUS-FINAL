/*
  Shot Clock TOP  v13.0  —  Panel 3  (64 × 16 px)
  ─────────────────────────────────────────────────────────────────────────
  Layout (top panel):
    Shows "SHOT CLOCK" label in small font (size 1, centred).
    Color: BLUE normal → RED when secs ≤ 5.

  Changes vs v12.0:
    • Replaced large digit top-half with small "SHOT CLOCK" label only.
      Large bold digit is now shown entirely on Slave 4 (bottom panel).
*/
...
void drawLabel(int secs) {
    ...
    // "SHOT CLOCK" — size 1 (8px tall), centred vertically in 16px panel
    display.setTextSize(1);
    display.setTextColor(col);
    const char* lbl = "SHOT CLOCK";
    int lw = (int)strlen(lbl) * 6;
    display.setCursor((64 - lw) / 2, 4);
    display.print(lbl);
}
...
void setup() {
    ...
    if (esp_now_init() != ESP_OK) {
        ...
        Serial.println("Shot clock top v13.0 ready");
    }
}

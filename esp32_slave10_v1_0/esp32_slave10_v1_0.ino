/*
  ╔══════════════════════════════════════════════════════════════╗
  ║  Slave 10  v1.0  —  Quarter  BOTTOM row                     ║
  ║  Hardware : 2 × P10 panels in one row  →  64 × 16 px        ║
  ╠══════════════════════════════════════════════════════════════╣
  ║  Content : size 3, cursor y=-16 → bottom 8 px visible       ║
  ║    quarter 1–4  →  "1".."4"                                 ║
  ║    quarter ≥ 5  →  "OT"                                     ║
...
void drawBottom(int quarter) {
    ...
    display.setCursor(tx, -16);               // bottom 8 px of char visible
    display.print(buf);
}
...
void setup() {
    ...
    if (esp_now_init() != ESP_OK) {
        ...
        Serial.println("Slave 10 v1.0 ready — 64x16 Quarter bottom row");
    }
}

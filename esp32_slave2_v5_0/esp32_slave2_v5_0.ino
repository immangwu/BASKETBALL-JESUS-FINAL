/*
  Slave 2 v5.0 — TOL + Possession Display
  ─────────────────────────────────────────────────────────────────────────
  Hardware : 4 panels wide × 2 tall  (128 × 32 px)

  Column layout (new wider zones so "TIME OUT" fits on one line):
    x=  0.. 45 (46 px) : "TIME OUT" label (top) + timeout dots (bottom) — Team A
    x= 46.. 63 (18 px) : ◄ left-pointing arrow if possession == 'A'
    x= 64.. 81 (18 px) : ► right-pointing arrow if possession == 'B'
    x= 82..127 (46 px) : "TIME OUT" label (top) + timeout dots (bottom) — Team B

  Changes vs v4.0:
    • Label zones widened from 32→46 px to fit "TIME OUT" on a single line.
    • Arrow zones narrowed from 32→18 px; triW=14, triH=14.
    • "TIME" + "OUT" two-line label replaced with single-line "TIME OUT".
*/
...
// Dot parameters — 5×12 px, 1 px gap, max 5
#define DOT_W      5
#define DOT_H     12
#define DOT_GAP    1
#define DOT_MAX    5
...
typedef struct __attribute__((packed)) {
    ...
    char marketingText[32];   // must match master BoardData
} BoardData;
...
void drawTolZone(int timeoutsA, char possession, int timeoutsB) {
    ...
    // Team A — single-line "TIME OUT" + dots
    {
        const char* lbl = "TIME OUT";
        int w  = strPixelWidth(lbl, 8);
        int x  = TOL_A_X + max(0, (TOL_A_W - w) / 2);
        drawStringRemapped(x, 1, lbl, 8, SystemFont5x7);
        int x0 = TOL_A_X + max(0, (TOL_A_W - DOT_ROW_W) / 2);
        for (int d = 0; d < timeoutsA && d < DOT_MAX; d++)
            drawDot(x0 + d * (DOT_W + DOT_GAP), DOT_VY);
    }

    // Team B — single-line "TIME OUT" + dots
    {
        const char* lbl = "TIME OUT";
        int w  = strPixelWidth(lbl, 8);
    ...
    // Possession arrows — triW=14, triH=14, centred in 18-px zones
    const int triW = 14, triH = 14;
    const int vy0  = (32 - triH) / 2;   // = 9
    const int half = triH / 2;           // = 7
...
void setup() {
    ...
    dmd.clearScreen(true);
    dmd.selectFont(SystemFont5x7);
    dmd.drawString(0, 0, "WAIT", 4, GRAPHICS_NORMAL);
    waitMs(1200);
    drawTolZone(0, ' ', 0);
    WiFi.mode(WIFI_STA);
    ...
    if (esp_now_init() != ESP_OK) {
        ...
        Serial.println("TOL panel v5.0 ready");
    }
}

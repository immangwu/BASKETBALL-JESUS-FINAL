/*
  Slave 4  v1.1  —  Shot Clock BOTTOM panel  (32 × 32 px)
  ─────────────────────────────────────────────────────
...
//   size 2 = 12×16 px
//   ".t" = 2 chars × 12 px = 24 px → tx = (32-24)/2 = 4  (centred)
void drawBottom(int secs, int tenths) {
    ...
    else            snprintf(buf, sizeof(buf), ".%d", tenths);
...
void setup() {
    ...
    if (esp_now_init() != ESP_OK) {
        ...
        Serial.println("Slave 4 v1.1 ready — 32x32 bottom panel");
    }
}

/*
  Marketing Display  v3.0  —  Panel 10  (64 × 16 px)
  ─────────────────────────────────────────────────────────────────────────
  Receives marketingText from master and displays it.
  Text ≤ 10 chars  → static centred display
  Text  > 10 chars → scrolls right-to-left continuously
...
int   scrollX         = 64;      // start off-right edge
int   textPixelW      = 0;
unsigned long lastScrollMs = 0;
#define SCROLL_SPEED_MS 40       // px per 40 ms  ≈  25 px/s
...
void applyText(const char* txt) {
    ...
    textPixelW = len * 6;           // size-1 font: 6px per char
    scrollMode = (textPixelW > 64);
    scrollX    = 64;                // reset scroll position
}
...
void stepScroll() {
    ...
    scrollX--;
    if (scrollX < -textPixelW) scrollX = 64;   // wrap around
}
...
void setup() {
    ...
    if (esp_now_init() != ESP_OK) {
        ...
        Serial.println("Marketing panel v3.0 ready");
    }
}

/*
  Shot Clock BOTTOM  v4.0  —  Panel 4  (64 × 16 px)
  ─────────────────────────────────────────────────────────────────────────
  Changes vs v3.0:
    • Tenths threshold: ≤ 4 s (was ≤ 5 s).  Color turns RED at ≤ 5 s.
    • Slave-side local timer with master-sync every packet.
    • No "Slave" word in startup message.

  Bottom half of large digit (size 3, cursor y=-16).
  Tenths ".X" subscript shown when secs ≤ 4.
*/
...
void drawBottom(int secs, int tenths) {
    ...
    // Tenths subscript — ≤ 4 s only
    if (secs <= 4) {
        char sub[4];
        snprintf(sub, sizeof(sub), ".%d", tenths);
...
void setup() {
    ...
    if (esp_now_init() != ESP_OK) {
        ...
        Serial.println("Shot clock bottom v4.0 ready");
    }
}

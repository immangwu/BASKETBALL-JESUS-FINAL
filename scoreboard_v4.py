#!/usr/bin/env python3
"""
Basketball Scoreboard v4.0
New vs v3.0:
  - YES VISIBLE button  : locks clocks until board confirmed
  - START MATCH button  : resets to Q1 + auto-starts clock
  - Smart serial detect : prefers ttyUSB/ttyACM on Pi, COM on Windows
  - GPIO Team A buttons : +1/+2/+3 score (pins 4,5,6) + foul+ (pin 12)
  - Event name 16 char  : visible counter in UI
  - Team name 12 char   : visible counter; no VS / no arrows
  - Clock SET dialog    : only + and − per minute / per second
  - Quick presets       : 2 min, 5 min, 8 min, 10 min
  - Quarter auto-flow   : Q1 auto-starts; breaks auto-run; Q2-Q4 wait START
  - Timeout rules       : Q1/Q2 = 2, Q3 = 3, Q4 starts 3 → 2 after 2 min
"""
...
try:
    ...
    _all_pins = [PIN_GCC_START, PIN_GCC_STOP, PIN_SCC_24, PIN_SCC_START,
                 PIN_SCC_STOP, PIN_SCC_14,
                 PIN_A_SCORE1, PIN_A_SCORE2, PIN_A_SCORE3, PIN_A_SCOREM1,
                 PIN_A_FOUL_P, PIN_A_FOUL_M, PIN_A_POSS,
                 PIN_B_SCORE1, PIN_B_SCORE2, PIN_B_SCORE3, PIN_B_SCOREM1,
                 PIN_B_FOUL_P, PIN_B_FOUL_M, PIN_B_POSS]
    for _p in _all_pins:
    ...
    for _pin, _sig in _map:
        GPIO.add_event_detect(_pin, GPIO.FALLING,
                              callback=lambda _, s=_sig: s.emit(), bouncetime=200)
    GPIO_OK = True
...
class ScoreboardApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self._build_ui()
        self._connect_gpio()
        self.setWindowTitle("Basketball Scoreboard v4.0")
        self.showMaximized()
    ...
    def _connect_gpio(self):
        ...
        gpio_sig.a_score1.connect(lambda: self._add_score("A", 1))
        gpio_sig.a_score2.connect(lambda: self._add_score("A", 2))
        gpio_sig.a_score3.connect(lambda: self._add_score("A", 3))
        gpio_sig.a_score_m1.connect(lambda: self._add_score("A", -1))
        gpio_sig.a_foul_p.connect(lambda: self._delta_fouls("A", 1))
        gpio_sig.a_foul_m.connect(lambda: self._delta_fouls("A", -1))
        gpio_sig.a_poss.connect(lambda: self._set_poss("A"))
        gpio_sig.b_score1.connect(lambda: self._add_score("B", 1))
        gpio_sig.b_score2.connect(lambda: self._add_score("B", 2))
        gpio_sig.b_score3.connect(lambda: self._add_score("B", 3))
        gpio_sig.b_score_m1.connect(lambda: self._add_score("B", -1))
        gpio_sig.b_foul_p.connect(lambda: self._delta_fouls("B", 1))
        gpio_sig.b_foul_m.connect(lambda: self._delta_fouls("B", -1))
        gpio_sig.b_poss.connect(lambda: self._set_poss("B"))
    ...
    def _make_scoreboard_tab(self):
        ...
        # ── Banner: YES VISIBLE + START MATCH ─────────────────────────────
        banner = QHBoxLayout()
        banner.setSpacing(8)
        ...
        rst_slaves_btn = QPushButton("↺  Reset All Slaves")
        rst_slaves_btn.setFixedHeight(54)
        rst_slaves_btn.setStyleSheet(
            f"background:#1e1b4b;color:#c7d2fe;font-size:15px;font-weight:bold;"
            f"border-radius:10px;border:1px solid #4338ca;"
        )
        rst_slaves_btn.clicked.connect(self._reset_slaves)
        banner.addWidget(rst_slaves_btn, 1)

        desktop_btn = QPushButton("⊟  Desktop")
        desktop_btn.setFixedHeight(54)
        desktop_btn.setStyleSheet(
            f"background:#1c1917;color:#a8a29e;font-size:15px;font-weight:bold;"
            f"border-radius:10px;border:1px solid #57534e;"
        )
        desktop_btn.clicked.connect(self.showMinimized)
        banner.addWidget(desktop_btn, 1)
        ...
        self.event_char_lbl = QLabel(f"{len(self.event_name)}/16")
        self.event_char_lbl.setStyleSheet(f"color:{TEXT3};font-size:12px;min-width:36px;")
    ...
    def _make_team_panel(self, side):
        ...
        char_lbl = QLabel("12")
        char_lbl.setStyleSheet(f"color:{TEXT3};font-size:11px;min-width:28px;")
        if is_a:
    ...
    def _make_center_panel(self):
        ...
        for b in [self.poss_a_btn, self.poss_none_btn, self.poss_b_btn]:
            b.setCheckable(True); b.setFixedHeight(50)
    ...
    def _on_event_changed(self, t):
        self.event_name = t
        self.event_char_lbl.setText(f"{len(t)}/16")
    ...
    def _update_team_char_lbl(self, side, t):
        ...
        remaining = 12 - len(t)
        lbl.setText(f"{remaining}")
        lbl.setStyleSheet(
            f"color:{'#f87171' if remaining <= 2 else TEXT3};font-size:11px;min-width:28px;"
        )
    ...
    def _open_kb_event(self):
        text, ok = VirtualKeyboard.getText(self, "Enter Event Title", self.event_name, 16)
        if ok:
    ...
    def _open_kb_team(self, side):
        ...
        text, ok = VirtualKeyboard.getText(self, f"Enter Team {side} Name", current, 12)
        if ok:
    ...
    # ─────────────────────────────────────────────────────────────────────
    def _send_name(self):
        pkt = f"N,{self.event_name},{self.team_a},{self.team_b},0\n"
    ...
    def _reset_slaves(self):
        if not self.serial_port or not self.serial_port.is_open:
            self._log("⚠ Serial not connected — cannot reset slaves", "warn"); return
        try:
            self.serial_port.write(b"R\n")
            self.serial_port.flush()
            self._log("↺ Reset command sent to all slaves", "success")
        except Exception as e:
            self._log(f"❌ {e}", "error")
            self._set_serial_ui(False, "Write error")
            self.serial_port = None
...
# ─────────────────────────────────────────────────────────────────────────────
#  SPLASH SCREEN  —  video → logo image → main window
# ─────────────────────────────────────────────────────────────────────────────
import os

_BASE       = os.path.dirname(os.path.abspath(__file__))
_VIDEO_PATH = os.path.join(_BASE, "PRODUCT LAUNCH",
                           "WhatsApp Video 2026-05-15 at 9.38.33 PM.mp4")
_LOGO_PATH  = os.path.join(_BASE, "PRODUCT LAUNCH",
                           "WhatsApp Image 2026-05-15 at 12.24.19 PM.jpeg")

LOGO_SHOW_MS = 2500   # how long to show logo after video ends


class SplashScreen(QWidget):
    """Plays intro video fullscreen, then shows logo for 2.5 s, then launches app."""
    ...
    def __init__(self):
        ...
        # ── layout: stacked video widget + logo label (only one visible at a time)
        ...
        self._layout.addWidget(self._video_w)

        # logo label (hidden until video ends)
        self._logo_lbl = QLabel()
        self._logo_lbl.setAlignment(Qt.AlignCenter)
        self._logo_lbl.setStyleSheet("background:black;")
        self._logo_lbl.hide()
        self._layout.addWidget(self._logo_lbl)
        ...
        if os.path.exists(_VIDEO_PATH):
            self._player.setMedia(
                QMediaContent(QUrl.fromLocalFile(_VIDEO_PATH))
            )
            self._player.play()
        else:
            # video file missing — skip straight to logo
            self._show_logo()
    ...
    def _on_player_error(self):
        # if GStreamer can't play the file, skip to logo
        self._show_logo()
    ...
    def _show_logo(self):
        ...
        if os.path.exists(_LOGO_PATH):
            ...
            self._logo_lbl.setText("JESUS HIVE")
            self._logo_lbl.setStyleSheet(
                "background:black; color:white; font-size:48px; font-weight:bold;"
            )

        self._logo_lbl.show()
        QTimer.singleShot(LOGO_SHOW_MS, self._finish)
...
def main():
    ...
    def launch():
        win = ScoreboardApp()
        win.setWindowIcon(QIcon(_LOGO_PATH) if os.path.exists(_LOGO_PATH) else QIcon())
        win.showMaximized()
        app._main_win = win   # keep reference so GC doesn't collect it

    splash.on_done(launch)
    sys.exit(app.exec_())

/*
  Slave 4  v1.0  —  Shot Clock BOTTOM panel  (32 × 32 px)
  ─────────────────────────────────────────────────────
...
// ── Draw (Slave 4 = BOTTOM half of combined display) ─────────────────────────
//   ≥10 s : show UNITS digit  (e.g. "4" from "24")
//   < 10 s : show ".tenths"   (e.g. ".5" from "9.5")
//
...
void drawBottom(int secs, int tenths) {
    ...
    char buf[4];
    if (secs >= 10) {
        snprintf(buf, sizeof(buf), "%d", secs % 10);   // units digit only
    } else {
        snprintf(buf, sizeof(buf), ".%d", tenths);      // ".t"
    }

    // centre in 32 px:
    //   single char (24 px wide) → x = (32-24)/2 = 4
    //   ".t"  (2 chars = 48 px wide) → x = 0 (slight clip is fine, or use size 3)
    int tw = (int)strlen(buf) * 24;
    int tx = max(0, (32 - tw) / 2);
...
void setup() {
    ...
    Serial.print("Slave4 MAC: "); Serial.println(WiFi.macAddress());
    ...
    if (esp_now_init() != ESP_OK) {
        ...
        Serial.println("Slave 4 v1.0 ready — 32x32 bottom panel");
    }
}

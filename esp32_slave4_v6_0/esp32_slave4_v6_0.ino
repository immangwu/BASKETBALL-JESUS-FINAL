/*
  Shot Clock BOTTOM  v6.0  —  Panel 4  (64 × 16 px)
  ─────────────────────────────────────────────────────────────────────────
  Layout (bottom panel):
    Shows shot clock digit in bold size-2 font (16px tall — fills panel).
    When secs ≤ 4: digit shrinks to size 1 and tenths ".X" shown beside it.
    Color: BLUE normal → RED when secs ≤ 5.

  Changes vs v5.0:
    • Replaced split large-digit bottom-half with full bold size-2 digit.
    • Tenths subscript shown when secs ≤ 4 (both size 1).
*/
...
void drawBold(int secs, int tenths) {
    ...
    if (secs > 4) {
        // Bold size-2 digit — 16px tall, fills panel height
        display.setTextSize(2);
        display.setTextColor(col);
        char buf[3];
        snprintf(buf, sizeof(buf), "%d", secs);
        int tw = (int)strlen(buf) * 12;
        display.setCursor((64 - tw) / 2, 0);
        display.print(buf);
    } else {
        // Size-1 digit + tenths side by side, centred
        char ss[3], tt[4];
        ...
        display.setTextColor(col);
        display.setCursor(x0, 4);
        display.print(ss);
        display.setCursor(x0 + (int)strlen(ss) * 6, 8);   // tenths as subscript
        display.print(tt);
    }
}
...
void setup() {
    ...
    if (esp_now_init() != ESP_OK) {
        ...
        Serial.println("Shot clock bottom v6.0 ready");
    }
}
...
void loop() {
    scanIfNeeded();
    tickShot();
    drawBold(localShotSecs, localShotTenths);

...
// x=0..63 has no physical panels (dark)
// Physical layout:  TeamA-TOL | TeamA-Arrow | TeamB-Arrow | TeamB-TOL
// Code x         :   64.. 95 |  96..127   |  128..159  | 160..191
#define TOL_A_X   64   // Team A timeout col  ( 64.. 95, 32 px)
#define TOL_A_W   32
#define ARR_A_X   96   // Team A arrow col   ( 96..127, 32 px) ◄ points left toward Team A
#define ARR_B_X  128   // Team B arrow col   (128..159, 32 px) ► points right toward Team B
#define ARR_W     32
#define TOL_B_X  160   // Team B timeout col (160..191, 32 px)
#define TOL_B_W   32

// Dot parameters — 5 wide × 12 tall, 1 px gap, max 5  (29 px wide fits in 32-px col)
// TOL label : VY=0..6  (row 5, upper panel)
// Dots      : VY=18..29 (row 6, lower panel — centred: 16+(16-12)/2=18)
#define DOT_W      5
#define DOT_H     12
#define DOT_GAP    1
#define DOT_MAX    5
#define DOT_ROW_W  (DOT_MAX * DOT_W + (DOT_MAX - 1) * DOT_GAP)   // 29 px wide
#define DOT_VY    18   // row 6 lower panel, centred vertically
...
// ── Draw one timeout dot (10×10) in the TOL zone ─────────────────────────────
// filled=true: solid square; filled=false: hollow border only
// Dot: DOT_W wide × DOT_H tall. filled=solid, else hollow border.
...
// ── TOL zone (rows 5+6) — 4 panels only, x=64..191 ──────────────────────────
//   [TOL-A x=64..95][ARR-A x=96..127][ARR-B x=128..159][TOL-B x=160..191]
//   x=0..63 has no physical panels
//   TOL label : VY=0..6  (row 5, upper panel)
//   5 dots    : VY=18..29 (row 6, lower panel, directly below label)
//   ARR-A ◄  : right-justify → apex points LEFT toward Team A
//   ARR-B ►  : left-justify  → apex points RIGHT toward Team B
void drawTolZone(int timeoutsA, char possession, int timeoutsB) {
    ...
    // Label in row 5 (VY=0, no offset). Dots in row 6 (DOT_VY, minus ROW6_X_OFFSET).
    {
        const char* lbl = "TOL";
        int w  = strPixelWidth(lbl, 3);
        int x  = TOL_A_X + max(0, (TOL_A_W - w) / 2);
        drawStringRemapped(x, 0, lbl, 3, SystemFont5x7, Y_ROW6);
        int x0 = (TOL_A_X - ROW6_X_OFFSET) + max(0, (TOL_A_W - DOT_ROW_W) / 2);
    ...
    // ── TOL label + 5 dots — Team B ──────────────────────────────────────────
    {
        ...
        drawStringRemapped(x, 0, lbl, 3, SystemFont5x7, Y_ROW6);
        int x0 = (TOL_B_X - ROW6_X_OFFSET) + max(0, (TOL_B_W - DOT_ROW_W) / 2);
        for (int d = 0; d < DOT_MAX; d++)
    ...
    // ── ◄ Team A arrow — right-justify → apex at LEFT ────────────────────────
    if (possession == 'A') {
        int x0 = (ARR_A_X - ROW6_X_OFFSET) + (ARR_W - triW) / 2;
        for (int r = 0; r < triH; r++) {
            int rowW   = triW - (abs(r - half) * triW / half);
            if (rowW < 1) rowW = 1;
            int startX = x0 + triW - rowW;
            for (int c = 0; c < rowW; c++)
                writePixelRemapped(startX + c, vy0 + r, Y_ROW6, 1);
        }
    }

    // ── ► Team B arrow — left-justify → apex at RIGHT ────────────────────────
    if (possession == 'B') {
        int x0 = (ARR_B_X - ROW6_X_OFFSET) + (ARR_W - triW) / 2;
        for (int r = 0; r < triH; r++) {
            int rowW = triW - (abs(r - half) * triW / half);
            if (rowW < 1) rowW = 1;
            for (int c = 0; c < rowW; c++)
                writePixelRemapped(x0 + c, vy0 + r, Y_ROW6, 1);
        }
    }
}

/*
  Slave 3 v1.1 — Fouls A | Shot Clock | Fouls B
  ─────────────────────────────────────────────
  ...
  Each zone (64×32):
    ...
    y= 8..31 : number textSize=3 (18×24 px) — centred in zone
    Shot clock < 10s: seconds textSize=3 at y=8, tenths textSize=2 at y=16 (lower baseline)
*/
...
void drawShotClockZone(int secs, int tenths) {
    ...
    if (secs >= 10) {
        ...
        // Large seconds + smaller tenths at lower baseline (≈40% down)
        ...
        display.setCursor(x0 + wSS, 16);   // lower baseline for tenths
        display.print(tt);
    }
}
...
void setup() {
    ...
    if (esp_now_init() != ESP_OK) {
        ...
        Serial.println("Slave 3 v1.1 ready — 192x32");
    }
}
